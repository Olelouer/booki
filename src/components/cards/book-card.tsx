    ...
    <article className="group relative flex h-full flex-col gap-2.5">
      ...
      <div className="flex grow flex-col justify-between">
        <div className="flex flex-col gap-2.5">
          <h2 className="line-clamp-2 font-serif text-lg">
            <Link
              to={`/book/${book.id}`}
              className="transition-colors group-hover:text-red-900 after:absolute after:inset-0 after:content-['']"
            >
              {book.title}
            </Link>
          </h2>
          ...
          {(book.publishedDate || hasPages) && (
            <div className="flex flex-wrap gap-1.5 text-[13px] text-stone-700">
              ...
              {hasPages && <span>{book.pageCount} p.</span>}
            </div>
          )}
          ...
          {book.categories && book.categories.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {book.categories.map((category) => {
                return <Tablet text={category} key={category} />;
              })}
            </div>
          )}
        </div>
