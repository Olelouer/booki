...
export function getBooks(): GetBooksResult {
  const message =
    "Une erreur est survenue lors de la lecture de la bibliothèque";
  try {
...
export function getYearPagesCount(books: Book[]): number {
  return books.reduce((acc, book) => {
    if (
      book.startedAt &&
      book.currentPage &&
      book.startedAt.slice(0, 4) === new Date().getFullYear().toString()
    ) {
